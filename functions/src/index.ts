...
async function promoteDueUpcomingUploads(): Promise<number> {
  const nowMs = Date.now();
...
export const schedulePromoteUpcomingUploads = onSchedule({ schedule: 'every 10 minutes' }, async () => {
    await promoteDueUpcomingUploads();
  });
